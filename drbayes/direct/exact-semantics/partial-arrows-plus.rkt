...
(: tree-left (Branch-Trace -> Branch-Trace))
(define (tree-left t)
  (cond [(trace-node? t)  (trace-node-fst t)]
        [else  'leaf]))

(: tree-right (Branch-Trace -> Branch-Trace))
(define (tree-right t)
  (cond [(trace-node? t)  (trace-node-snd t)]
        [else  'leaf]))
...
(define-syntax-rule (define-transformed-arrow
  ...
  (begin
    (define-type (Out-Arrow X Y) (In-Arrow (Pair Branch-Trace X) Y))
    ...
    (: tree-left/a (In-Arrow Branch-Trace Branch-Trace))
    (define tree-left/a
      ((inst arr/a Branch-Trace Branch-Trace) tree-left))
    
    (: tree-right/a (In-Arrow Branch-Trace Branch-Trace))
    (define tree-right/a
      ((inst arr/a Branch-Trace Branch-Trace) tree-right))
    
    (: first/a (All (X Y Z) ((In-Arrow X Y) -> (In-Arrow (Pair X Z) (Pair Y Z)))))
    (define (first/a f)
      (((inst fst/a X Z) . >>>/a . f) . &&&/a . (inst snd/a X Z)))
    ...
    (define (f1 . >>>/a+ . f2)
      (define id/a ((inst arr/a (Pair Branch-Trace X) (Pair Branch-Trace X)) id))
      ((id/a . &&&/a . (((inst first/a Branch-Trace Branch-Trace X) tree-left/a) . >>>/a . f1))
       . >>>/a . (((inst first/a (Pair Branch-Trace X) Branch-Trace Y)
                   ((inst fst/a Branch-Trace X) . >>>/a . tree-right/a))
                  . >>>/a . f2)))
    
    (: &&&/a+ (All (X Y Z) ((Out-Arrow X Y) (Out-Arrow X Z) -> (Out-Arrow X (Pair Y Z)))))
    (define (f1 . &&&/a+ . f2)
      (define first (inst first/a Branch-Trace Branch-Trace X))
      (((first (arr/a tree-left)) . >>>/a . f1) . &&&/a . ((first (arr/a tree-right)) . >>>/a . f2)))
    
    (: ifte/a+ (All (X Y) ((Out-Arrow X Boolean) (Out-Arrow X Y) (Out-Arrow X Y) -> (Out-Arrow X Y))))
    (define (ifte/a+ f1 f2 f3)
      (define first (inst first/a Branch-Trace Branch-Trace X))
      (ifte/a ((first (arr/a tree-left)) . >>>/a . f1)
              ((first (arr/a (compose tree-left tree-right))) . >>>/a . f2)
              ((first (arr/a (compose tree-right tree-right))) . >>>/a . f3)))
    
    (: lazy/a+ (All (X Y) ((-> (Out-Arrow X Y)) -> (Out-Arrow X Y))))
    (define (lazy/a+ f) (lazy/a f))
    
    (: branch/a+ (All (X) (-> (Out-Arrow X Boolean))))
    (define (branch/a+)
      ((inst fst/a Branch-Trace X) . >>>/a . (π/a j0)))
    
    (: ifte-conv/a+ (All (X Y) ((Out-Arrow X Boolean) (Out-Arrow X Y) (Out-Arrow X Y)
                                                      -> (Out-Arrow X Y))))
    (define (ifte-conv/a+ f1 f2 f3)
      (define first (inst first/a Branch-Trace Branch-Trace X))
      (ifte/a ((((first (arr/a tree-left)) . >>>/a . f1)
                . &&&/a . ((inst branch/a+ X)))
               . >>>/a . agrees/a)
...
(define (ap/bot+ f x)
  (define B (set-image f (set-product some-traces (set x))))
